...
export const Adsets = ({
  ...
  const [selectedAdSet, setSelectedAdSet] = useState<{ adSetId: string; platform: string | null } | null>(null);
  ...
  const { data: ads, isLoading: isAdsLoading } = useQuery({
    ...
    queryFn: () => fetchAdsApi(user.role, user.user_id, selectedAdSet?.platform || "", selectedAdSet?.adSetId),
  ...
  const formatLocations = (geoLocations: any) => {
    ...
    const cities = geoLocations.custom_locations
      .map((loc: any) => cityIdToName[loc.primary_city_id] || `Lat: ${loc.latitude}, Lon: ${loc.longitude}`)
  ...
  return (
    <>
    {isCreateAdSetModalOpen && <CreateAdSets />}
                        ...
                        <Dialog key={adSet.id}>
                          <DialogTrigger asChild>
                            <div
                              ...
                              onClick={() => setSelectedAdSet({ adSetId: adSet.id, platform:adSet.platform })}
                              >
                              ...
                              <p className="text-xs text-gray-600">
                                Daily Budget: ${(parseInt(adSet.daily_budget) / 100).toFixed(2)}
                              </p>
                              <p className="text-xs text-gray-600">
                                Targeting: Ages {adSet?.targeting?.age_min}-{adSet?.targeting?.age_max}, {formatLocations(adSet?.targeting?.geo_locations)}
                              </p>
                              {adSet.targeting.flexible_spec && (
                                <p className="text-xs text-gray-600">
                                  ...
                                  {adSet?.targeting?.flexible_spec[0]?.work_positions
                                    ...
                                    .join(", ") +
                                    (adSet?.targeting?.flexible_spec[0]?.work_positions?.length > 3 ? "..." : "")
                                    : ""}
                                </p>
                              )}
                              <p className="text-xs text-gray-600">
                          ...
                          <DialogContent className="sm:max-w-[600px]">
                            <DialogHeader>
                              <DialogTitle>{adSet?.name} - Details</DialogTitle>
                            </DialogHeader>
                              < AdsListWithInsights ads={ads} isAdsLoading={isAdsLoading}/>
                          </DialogContent>
                        </Dialog>
                      ))
                    )}
