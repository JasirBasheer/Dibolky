...
const socialMediaConfigs: Record<string, SocialMediaConfig> = {
  instagram: {
    ...
    connectEndpoint: '/api/agency/client/connect/instagram'
  },
  facebook: {
    name: 'facebook',
    buttonText: 'Connect Facebook',
    connectEndpoint: '/api/agency/client/connect/facebook'
  },
  x: {
    name: 'x',
    buttonText: 'Connect X',
    connectEndpoint: '/api/agency/client/connect/x'
  }
};
  ...
  const handleCallback = async (token: string, provider: string): Promise<any> => {
    try {
      ...
      const response = await axios.post(
        `/api/agency/savePlatFormToken/${provider}/${selectedClient && selectedClient != "" ? selectedClient : "Agency"}`,
        { token }
      );
      if(response){
