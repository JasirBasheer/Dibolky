...
import { useAgoraRtmLogin } from "@/hooks/user-agora";
...
export const ProtectedRoute = ({
  ...
  const [userIdForRtm, setUserIdForRtm] = useState<string | undefined>(
    undefined
  );
  useAgoraRtmLogin(userIdForRtm);
    ...
    try {
      ...
      if (response.data) {
        ...
        if (role == "agency") {
          ...
          dispatch(
            setUser({
              ...
              facebookAccessToken:
                res.data.details?.social_credentials?.facebook?.accessToken ||
                "",
              instagramAccessToken:
                res.data.details?.social_credentials?.instagram?.accessToken ||
                "",
              profile: res.data.details?.profile || "",
              permissions: res.data.details?.permissions || [],
          ...
          userIdToUse = res.data.details?._id;
        } else if (
          role == "client" &&
          response?.data.details.orgId &&
        ...
        if (response.data.role !== role) {
          navigate(
            roleRedirects[response.data.role as keyof typeof roleRedirects]
          );
        }
      }
    } catch (error: unknown) {
