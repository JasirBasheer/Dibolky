...
export default class CompanyService implements ICompanyService {
  ...
  async verifyOwner(id: string): Promise<any> {
    try {
      return await this.companyRepository.findCompanyWithId(id)
    } catch (error) {
      throw error
    }
  }

  async companyLoginHandler(email: string, password: string): Promise<any> {
