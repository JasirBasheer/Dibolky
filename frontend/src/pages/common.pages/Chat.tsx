...
import { ChatItemProps, IChat, IChatUser, Participant } from '@/types/chat.types';
...
const Chat: React.FC<{ user: IChatUser, ownerId: string, isAdmin?: boolean }> = ({ user, ownerId, isAdmin }) => {
  ...
  //   queryFn: () => {
  //     return fetchMessagesApi(ownerId)
  //   },
  ...
  const fetchAllChats = async () => {
    if (!ownerId) return;
    try {
      const response = await fetchAllChatsApi(ownerId)
  ...
  useEffect(() => {
    ...
    const handleNewMessage = ({ newMessage, chat_id, participants }: { newMessage: IMessage, chat_id: string, participants: Participant[] }) => {
      const isParticipant = participants.some((p: Participant) => p.userId == ownerId);
    ...
    const handleNewChat = ({ newChat }: { newChat: IChat }) => {
      const isParticipant = newChat.participants!.some((p: Participant) => p.userId === ownerId);
      if (!isParticipant) return;
      ...
      setChats(prevChats => {
        ...
        const formattedChat = {
          ...
          name: newChat.participants!.length < 3
            ? newChat.participants?.find((p: Participant) => p.userId !== ownerId)?.name
            : newChat.name
        };
    ...
    const handleOnGroupCreated = ({ group }: { group: IChat }) => {
      const isParticipant = group?.participants?.some(
        (participant: Participant) => participant.userId == ownerId
      );
      if (isParticipant) {
        new Audio(newMessageNotification).play()
